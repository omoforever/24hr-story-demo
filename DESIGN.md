...
**[2026-09-08, revised same day] Viewer frame** — one set of styles for every breakpoint and orientation: `width: 100%`, `maxWidth: min(420px, calc(100dvh * 9 / 16))`, `aspectRatio: 9/16`, centred on `common.black`. The cap never binds on a tall phone, so mobile is full-screen; desktop and landscape collapse to a phone-shaped frame, with no `useMediaQuery` branch. **The height term is load-bearing** — with a plain `maxHeight: 100%` instead, `max-height` beats `aspect-ratio` on a short viewport and the frame flattens into a landscape box. `dvh` not `vh`, so collapsing mobile browser chrome doesn't clip it. Image is `objectFit: contain` — never `cover`, which would crop against the 1080×1920 work `image.ts` does.

**[2026-09-08] Safe areas** — `layout.tsx` sets `viewportFit: "cover"` so the viewer runs edge to edge under the notch, which means anything pinned to an edge needs the matching inset. The viewer header uses `calc(12px + env(safe-area-inset-top))`; add the equivalent for any future bottom-anchored control.
